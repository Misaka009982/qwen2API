...
def _parse_tool_calls(answer: str, tools: list, *, emit_logs: bool):
    ...
    def _make_tool_blocks(parsed_calls: list[dict[str, Any]], prefix=""):
    ...
    if tc_matches:
        try:
            ...
            prefix = answer[:tc_matches[0].start()].strip()
            _log_info(f"[ToolParse] ✓ ##TOOL_CALL## 格式: calls={[call['name'] for call in parsed_calls]!r}")
            return _make_tool_blocks(parsed_calls, prefix)
        except (json.JSONDecodeError, ValueError) as e:
            _log_warning(f"[ToolParse] ##TOOL_CALL## 格式解析失败: {e}, content={tc_matches[0].group(1)[:100]!r}")

    xml_matches = list(re.finditer(r'<tool_call>\s*(.*?)\s*</tool_call>', answer, re.DOTALL | re.IGNORECASE))
    if xml_matches:
        try:
            parsed_calls = []
            for match in xml_matches:
                ...
                if isinstance(inp, str):
                    try:
                        ...
                        inp = {"value": inp}
                parsed_calls.append({"name": obj.get("name", ""), "input": inp})
            prefix = answer[:xml_matches[0].start()].strip()
            _log_info(f"[ToolParse] ✓ XML格式 <tool_call>: calls={[call['name'] for call in parsed_calls]!r}")
            return _make_tool_blocks(parsed_calls, prefix)
        except (json.JSONDecodeError, ValueError) as e:
            _log_warning(f"[ToolParse] XML格式解析失败: {e}, content={xml_matches[0].group(1)[:100]!r}")
    ...
    if cb_m:
        try:
            ...
            return _make_tool_blocks([{"name": name, "input": inp}], prefix)
        except (json.JSONDecodeError, ValueError) as e:
    ...
    try:
        if stripped_clean.startswith('{') and stripped_clean.endswith('}'):
            ...
            if isinstance(obj, dict) and "name" in obj:
                ...
                return _make_tool_blocks([{"name": name, "input": inp}])
    except (json.JSONDecodeError, ValueError) as e:
...
class ToolSieve:
        ...
        try:
            ...
            if stop_reason == "tool_use":
                ...
                if tool_blocks:
                    ...
                    prefix = "".join(tb.get("text", "") for tb in text_blocks)
    ...
    def _looks_like_incomplete_tool_call(self, text: str) -> bool:
        ...
        return any(marker in masked for marker in markers)
