...
def has_invalid_textual_tool_contract(answer_text: str) -> bool:
    if not answer_text:
        return False
    masked_answer = mask_ignored_tool_syntax_regions(answer_text)
    if "##TOOL_CALL##" not in masked_answer and "<tool_call>" not in masked_answer:
        return False
...
def should_retry_textual_tool_contract(answer_text: str) -> bool:
    if not answer_text:
        return False
    masked_answer = mask_ignored_tool_syntax_regions(answer_text)
    if "##TOOL_CALL##" in masked_answer or "<tool_call>" in masked_answer:
        return True
    return False
    ...
    *,
    ...
    async for item in client.chat_stream_events_with_retry(
        ...
        if phase == "answer" and content:
            ...
            if request.tools:
                answer_text = "".join(answer_fragments)
                masked_answer_text = mask_ignored_tool_syntax_regions(answer_text)
                ...
                if "##TOOL_CALL##" in masked_answer_text or "<tool_call>" in masked_answer_text:
...
def retryable_usage_delta(prompt: str):
    return lambda execution, current_prompt=None: calculate_execution_usage(current_prompt or prompt, execution)["total_tokens"]
